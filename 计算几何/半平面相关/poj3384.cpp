...
int main()
{
    ...
    double r,d,tmp;
    Point p1,p2;
    freopen("poj3384.txt","r",stdin);
    freopen("poj3384ans.txt","w",stdout);
    while (scanf("%d%lf",&n,&r)!=EOF)
    {
        for (i=0;i<n;i++) scanf("%lf%lf",&p[i].x,&p[i].y);
        for (i=0;i<n;i++)
        {
            l[i]=Line(p[(i+1)%n],p[i]);  //原来点给出来是顺时针的
            l[i]=HalfPlaneMoveIn(l[i],r);
        }
        ...
        for (i=0;i<pn-1;i++)
            ...
            {
                tmp=len(p[i]-p[j]);
                if (tmp>=d)         //改成>=的目的是，一旦d=0，可以刷新
                {                   //因为我测了一组特殊数据，返回的点全部是相同的，但是pn>=3
                    d=tmp;
                    p1=p[i];
                    p2=p[j];
                }

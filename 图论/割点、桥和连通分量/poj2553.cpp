...
int n,m,ans[MaxN],flag[MaxN],pn; 

void AddPoint(int id)
{
	int i;
	for (i=1;i<=n;i++) if (grp[i]==id) flag[i]=1;
}

int main()
{
    freopen("poj2553.txt","r",stdin);
    freopen("poj2553ans.txt","w",stdout);
    int i,u,v;
    scanf("%d",&n); 
    while (n)
    {
        ...
        pn=0;
        for (i=1;i<=scc;i++) if (oud[i]==0) AddPoint(i);
        for (i=1;i<=n;i++) if (flag[i]==1) ans[++pn]=i;
		for (i=1;i<pn;i++) printf("%d ",ans[i]);
        printf("%d\n",ans[pn]); 
        scanf("%d",&n); 
    }
} 
